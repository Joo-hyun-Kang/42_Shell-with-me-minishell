...
/*   Updated: 2022/06/24 12:31:56 by jokang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sort_redir_command(t_argument **arg, t_lst *list_arg, t_lst *list_com)
{
    ...
    ft_copy_origin_command(arg, list_com);
	p = *arg;
	gt_file = NULL;
	while (p->next_token_type != PIPE && p->next_token_type != EOL)
	{
		...
		{
			p = p->next;
            ft_get_gt_variable(p, list_arg, &gt_file);
		}
		else if (p->next_token_type == LT || p->next_token_type == DLT)
		{
            token = p->next_token_type;
			p = p->next;
            ft_get_lt_dlt(p, list_arg, list_com, token);
		}
		else
			p = p->next;
	}
